...
class LoginRecuperar extends Util
{
	...
	public function montaHtmlLogin(){
		...
		if($_POST['log'] && $_POST['log'] != ''){
			$usuario = $rf;
			$api_url = getenv('SMEINTEGRACAO_API_URL') . '/api/v1/autenticacao/RecuperarSenha/usuario?sistema=2';
			...
			if($response['response']['code'] == 200){		
				...
				<script>Swal.fire({
					title: 'Email enviado com sucesso!',
  					icon: 'success',
					html:
					'Seu link de troca de senha dos sistemas da SME (Intranet, SGP e outros) foi enviado para <?= $userEmail; ?> , verifique sua caixa de entrada. Se você não reconhece ou não tem acesso a esse e-mail, solicite o reset de senha da Intranet, pedindo à equipe gestora ou ao POED que abra um chamado pelo site: <a href="https://suporte.sme.prefeitura.sp.gov.br">suporte.sme.prefeitura.sp.gov.br</a> (caminho: Portal de Chamados / Contas e Acessos /Alterar Senha-SGP). E, após seu primeiro acesso na Intranet, atualize o e-mail na tela de perfil.',
				})</script>
				...
				if($response['body'] == '"Usuário ou RF não encontrado"'){
					...
					<script>
						Swal.fire('Email não encontrado!', 'Você não tem um e-mail cadastrado para recuperar sua senha. Solicite o reset de senha da Intranet, pedindo à equipe gestora ou ao POED que abra um chamado pelo site: <a href="https://suporte.sme.prefeitura.sp.gov.br">suporte.sme.prefeitura.sp.gov.br</a> (caminho: Portal de Chamados / Contas e Acessos /Alterar Senha-SGP). Após seu primeiro acesso na Intranet, atualize o e-mail na tela de perfil.', 'warning');
					</script>
