...
class LoopNoticiasNoticiaPrincipal extends LoopNoticias
{
	...
	public function montaHtmlNoticiaPrincipal(){
		if (have_posts()):
			...
			echo '<article class="col-12 col-lg-8 content-article content-explica news-content">';
	...
	public function getDataPublicacaoAlteracao(){
		...
		echo '<p class="data"><span class="display-autor">Publicado em: '.get_the_date('d/m/Y G\hi').' | Atualizado em: '.get_the_modified_date('d/m/Y');
			...
			$term_obj_list = get_the_terms( get_the_ID(), 'categorias-noticias' );
			$i = 0;
			if($term_obj_list){
				echo " - em ";
