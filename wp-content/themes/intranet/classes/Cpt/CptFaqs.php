...
class CptFaqs extends Cpt
{
	...
	function filter_categ_portais($query) {
		...
		if(isset($_GET['categorias_portais']) && $_GET['categorias_portais'] != ''){
			$tax = array(
				array(
			...
			case 'categorias-portais':
				...
				if (!empty($categorias) && is_array($categorias)) {
					foreach ($categorias as $index => $categoria) {
						...
						echo $separator . '<a href="' . get_home_url() . '/wp-admin/edit.php?post_type=portais&categorias_portais=' . $categoria->term_id . '">' . $categoria->name . '</a>';
					}
				} else {
