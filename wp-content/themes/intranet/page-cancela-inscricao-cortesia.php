...
<?php 
	...
	//###### É NECESSÁRIO ADICIONAR UMA PÁGINA NO PAINEL DO WORDPRESS COM O NOME: CANCELA INSCRICAO CORTESIA (Ascendente - > Sorteio)
	$url = get_site_url();

	if ($_POST['idInscrito']){
	
        // Processa o cancelamento da inscrição
        $result = processar_devolucao_cortesia( $_POST['idInscrito'], $_POST['chave'] ?? '' );
        
        $mensagem = 'Sua inscrição foi cancelada com sucesso. Você não possui mais uma inscrição ativa vinculada.';
	...
	$inscrito = Envia_Emails_Sorteio_SME::get_inscrito_by_id($idInscrito, 'cortesias');

	if($inscrito || isset($inscrito[0]->post_id)){
...
?>
  ...
  <div class="container">
    ...
    <p class="description">
       ...
       <span class="semibold">Para cancelar sua inscrição referente a</span> <span class="bold"><?= $tituloEvento; ?></span><span class="semibold">, verifique seus dados e clique em Cancelar Inscrição.</span>
