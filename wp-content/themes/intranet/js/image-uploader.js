        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                ...
                Swal.fire({
					icon: 'warning',
					title: 'Atenção',
					text: 'O arquivo "' + file.name + '" não corresponde às extensões de arquivo aceitas: ' + plugin.settings.extensions.join('", "'),
				});
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                ...
                Swal.fire({
					icon: 'warning',
					title: 'Atenção',
					text: 'O arquivo "' + file.name + '" excede o tamanho de ' + plugin.settings.maxSize / 1024 / 1024 + 'Mb',
				});
                return false;
            }
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                ...
                Swal.fire({
					icon: 'warning',
					title: 'Atenção',
					text: 'O arquivo "' + file.name + '" não pode ser adicionado pois excede o limite de ' + plugin.settings.maxFiles + ' imagens.',
				});
                return false;
            }
