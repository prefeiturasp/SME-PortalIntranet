...
function aplicar_sancao_ajax() {
	...
	$tipo		   = isset($_POST['tipo']) ? sanitize_text_field( $_POST['tipo'] ) : 'sorteio';
	...
	if ( $acao == 'retorna_lista_sorteados' ) {
		...
		if(count($arrDados) < 1){
			...
			if(is_plugin_active('envia-email-sme/envia-email-sme.php')){
				...
				definir_prazo_expiracao_email_confirmacao( $participantesSelecionados, $tipo_prazo_confirmacao, $prazo_confirmacao, $prazoExpirado );

				foreach($arrEmails as $item){
					new Envia_Emails_Sorteio_SME($item['id'], null, $item['post_id'], $item['tipoEmail'], null, null, ['prazo_expirado' => $prazoExpirado]);
				}
			}
		}
...
function scripts_exportacao_inscritos_admin() {//** OK */
    ...
    if ($pagenow === 'post.php' && isset($_GET['post']) && $screen->post_type === 'post' ) {
...
function obter_proxima_data_sorteio( $post_id, $semana = true ) {
    ...
    if ( $evento_datas && $tipo_evento != 'periodo' ) {
...
function handle_enviar_instrucoes() {    
    ...
    if ($opcao === 'selecionados') { // Somente para selecionados
		...
		} elseif($opcao === 'geral' && $tipo_post === 'sorteio') {
			if ( $tipo_evento === 'periodo' ) {
				$arrDados = $wpdb->get_results(
					$wpdb->prepare("
						SELECT i.id, i.post_id
		...
		} elseif($opcao === 'geral' && $tipo_post === 'cortesias') {
    ...
    foreach ( $destinatarios_data as $destinatario ) {
        ...
        $wpdb->insert(
            ...
            [
				...
				'tipo_noticia'		  =>  $tipo_post === 'cortesias'? 'c' : 's'
            ]
        );
    }
...
/**
 * Retorna o prêmio do repeater "evento_premios" a partir da data
 *
 * @param int    $post_id
 * @param string $data_procurada (Y-m-d H:i:s)
 * @return string|null
 */
function get_premio_por_data($post_id, $data_procurada) {
    if (!$post_id || !$data_procurada) {
        return null;
    }

    $premios = get_field('evento_premios', $post_id);

    if (!$premios || !is_array($premios)) {
        return null;
    }

    foreach ($premios as $linha) {
        if (!empty($linha['data']) && $linha['data'] === $data_procurada) {
            return $linha['premio'] ?? null;
        }
    }
...
add_action('wp_ajax_confirmar_cancelar_presenca_sorteio', 'ajax_confirmar_cancelar_presenca_sorteio');
add_action('wp_ajax_nopriv_confirmar_cancelar_presenca_sorteio', 'ajax_confirmar_cancelar_presenca_sorteio');
function ajax_confirmar_cancelar_presenca_sorteio() {
	...
	$prazo_confirmacao = new DateTime( $inscricao[0]->prazo_confirmacao, $wp_timezone );
	$data = new DateTime( $inscricao[0]->data_sorteada );
	$tipo_sorteio = get_field( 'tipo_evento', $inscricao[0]->post_id );
	...
	if( $agora > $data && $tipo_sorteio != 'premio' ){ // Data de evento já passou
		...
		if ($tipo_sorteio == 'premio' ) {
			$mensagem = 'O prazo para confirmação expirou. Sentimos muito, mas a oportunidade vinculada a este sorteio foi disponibilizada para outro participante. Fique atento(a) aos próximos sorteios!';
		} else {
			$mensagem = 'O prazo para confirmar ou cancelar sua presença neste evento expirou. Sentimos muito, mas seu ingresso foi disponibilizado para outro participante. Fique atento(a) aos próximos sorteios!';
		}
