...
<div class="accordion-sorteio" id="lista-participantes-sorteados" data-post="<?php echo esc_html( get_the_ID() ); ?>">
    ...
    foreach ( $datas as $item ) :
        ...
        if ($tipo_evento == 'premio') {
            ...
            $data_sorteio_raw = $info_periodo_evento['data_sorteio'] ?? null;

            if (!empty($data_sorteio_raw)) {
                $data_sorteio = DateTime::createFromFormat('d/m/Y', trim($data_sorteio_raw));
            }

            // fallback: se não criou o objeto (campo vazio ou inválido)
            if (empty($data_sorteio) || $data_sorteio === false) {
                $data_sorteio = new DateTime(); // data atual
            }
