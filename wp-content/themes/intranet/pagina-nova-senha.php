...
<?php
    ...
    function valida_senha($senha){
        if(!preg_match('/^(?=.*\d)(?=.*[^a-zA-Z0-9])(?=.*[a-z])(?=.*[A-Z])[^\s]{8,12}$/',$senha)) {
           return false;
        } else {
            return true;
        }
    ...
    if($_GET['token'] && $_GET['token'] != ''){
        ...
        $api_url = getenv('SMEINTEGRACAO_API_URL') . '/api/v1/autenticacao/RecuperarSenha/token/validar';
        ...
        if($_POST['senha-nova'] != $_POST['senha-repita'] && $token){
            ...
            if(isset($_POST['senha-nova'])){
                ...
                if($token && $validar){
                    ...
                    // URL da API
                    $api_url = getenv('SMEINTEGRACAO_API_URL') . '/api/v1/autenticacao/AlterarSenha';
                    ...
                    $response = wp_remote_post( $api_url ,
                            array(
                                'headers' => array( 
                                    'x-api-eol-key' => getenv('SMEINTEGRACAO_API_TOKEN'), // Chave da API
                                ),
