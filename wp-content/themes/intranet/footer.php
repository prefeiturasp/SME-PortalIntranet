...
<script>
	...
	/*
	Bloco comentado devido uma incompatibilidade do bootstrap com os
	comportamentos sobrepostos pelo jQuery no menu principal em itens com dropdown
	jQuery( function ( $ ) {
...
<?php if($_GET['expirado'] == '1'):
		...
		if($_GET['evento'] && $_GET['evento'] != ''){
			...
			$postType = get_post_type( $_GET['evento'] );
		}

		if($postType == 'cortesias'){
			$texto = 'O período de inscrições para este evento foi encerrado. Fique de olho em nosso site para futuras oportunidades!';
		} else {
			if($dataSorteio != ''){
...
<?php endif; ?>
<?php if($_GET['expirado'] == '5'): ?>
	<script>
		Swal.fire({
			icon: 'warning',
			title: 'Atenção: esse link não está mais ativo porque você já confirmou ou cancelou sua presença.',
			text: 'Se já confirmou, fique tranquilo(a): em breve você receberá um novo e-mail com as instruções.',
		});
	</script>
<?php endif; ?>
...
<?php if($_GET['conf-expirado'] == '2') :
	...
	$tipo = $_GET['tipo'];
	if($tipo == 'premio'): 
		$mensagem = 'Este link não está mais ativo, pois o prazo para confirmação expirou. Sentimos muito, mas a oportunidade vinculada a este sorteio foi disponibilizada para outro participante. Fique atento(a) aos próximos sorteios!';
	elseif($tipo == 'cortesia'):
		$mensagem = 'Este link não está mais ativo, pois o prazo para confirmação expirou. A participação não foi confirmada dentro do prazo estabelecido e, por isso, a oportunidade foi disponibilizada novamente para inscrição, podendo ser ocupada por qualquer interessado.';
	else:
		$mensagem = 'Esse link não está mais ativo porque o prazo para confirmar sua presença neste evento expirou. Sentimos muito, mas seu ingresso foi disponibilizado para outro participante. Fique atento(a) aos próximos sorteios!';
	endif;
	?>
	<script>
		Swal.fire({
			icon: 'info',
			title: 'Atenção!',
			text: '<?php echo esc_html( $mensagem ); ?>',
		});
	</script>
<?php endif; ?>
...
<?php if($_GET['confirmacao'] == '2') :
	...
	$tipo = $_GET['tipo'];
	if($tipo == 'premio' || $tipo == 'cortesia'): 
		$mensagem = 'A oportunidade será disponibilizada para outro participante. Obrigado pela participação!';
	else:
		$mensagem = 'Seu ingresso será resorteado! Obrigado pelo participação!';
	endif;
	?>
	<script>
		Swal.fire({
...
<?php if($_GET['confirmacao'] == '3') :
	$mensagem = ( isset( $_GET['tipo'] ) && $_GET['tipo'] == 'cortesia' )
		? 'Esse link não está mais ativo porque você já confirmou ou cancelou sua participação. Se já confirmou, fique tranquilo(a): em breve você receberá um novo e-mail com as instruções.'
		: 'Esse link não está mais ativo porque você já confirmou ou cancelou sua presença. Se já confirmou, fique tranquilo(a): em breve você receberá um novo e-mail com as instruções.'
	?>
	<script>
		Swal.fire({
			icon: 'info',
		...
		jQuery(document).ready(function ($) {
			...
			$('#cancelarInscricao').on('click', function () {
				...
				$.post('/wp-admin/admin-ajax.php', {
					...
					if (response.success && response.data.sorteio_realizado) {
						return Swal.fire({
			...
			$('#cancelarInscricaoCortesia').on('click', function () {
				...
				$.post('/wp-admin/admin-ajax.php', {
					...
					if (response.success) {
						...
						if (dados.tipo_evento == 'data') {
							textoPorTipo = `Deseja cancelar sua inscrição do dia ${dados.data_formatada}?`;
						}

						if (dados.tipo_evento == 'periodo') {
							textoPorTipo = 'Deseja cancelar sua inscrição?';
						}

						if (dados.tipo_evento == 'premio') {
							textoPorTipo = `Deseja cancelar sua inscrição para o prêmio ${dados.premio}?`;
						}
						...
						Swal.fire({
							...
							html: `
								<p>${textoPorTipo}</p>
								<p>
									Seus dados serão removidos da listagem de inscritos.
									O item associado à sua inscrição será disponibilizado novamente, conforme prazo e disponibilidade.
								</p>
							`,
							icon: 'warning',
							showCancelButton: true,
			...
			function devolverCortesia(inscricaoId) {
				...
				if (inscricaoId) {
					$.ajax({
						...
						method: 'POST',
						data: {
							action: 'devolver_cortesia',
							inscricao_id: inscricaoId,
							nonce: '<?php echo wp_create_nonce("devolver_cortesia_nonce"); ?>'
						},
						success: function (response) {
							if (response.success) {
								Swal.fire({
